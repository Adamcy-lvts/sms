...
//             ->whereIn('category', ['admission', 'academic'])
//             ->get();

//         $data = $variables->mapWithKeys(function ($variable) use ($admission) {
//             // Extract model and field from mapping
//             [$model, $field] = explode('.', $variable->mapping);

//             // Get value based on model
//             $value = match ($model) {
//                 'admission' => $this->resolveValue($admission, $field),
//                 'student' => $this->resolveValue($admission->student, $field),
//                 'school' => $this->resolveValue($this->school, $field),
//                 default => null
//             };
...
//     protected function resolveValue($model, $field): mixed
//     {
//         if (!$model) {
//             return null;
//         }

//         // Handle relationship fields (ending with _name)
//         if (str_ends_with($field, '_name') && $field !== 'full_name') {
//             $relationName = Str::camel(str_replace('_name', '', $field));
//             $relatedModel = $model->{$relationName};

//             if ($relatedModel) {
//                 // For models with name attribute
//                 return $relatedModel->name ?? $relatedModel->title ?? null;
//             }

//             return null;
//         }

//         // Handle regular fields (including full_name)
//         return $model->$field;
//     }


//     private function getPrincipalName(): string
//     {
//         return $this->school->staff()
//             ->whereHas('designation', fn($q) => $q->where('name', 'Principal'))
//             ->first()?->full_name ?? 'Principal';
//     }
// }
